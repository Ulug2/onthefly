...
const getAllTrips = async (req, res) => {
    try {
        ...
        const results = await pool.query(
            'SELECT trips.* FROM trips INNER JOIN trips_destinations ON trips.id = trips_destinations.trip_id WHERE trips_destinations.destination_id = $1',
            [destination_id]
        )
...
const getAllDestinations = async (req, res) => {
    try {
        ...
        const results = await pool.query(
            'SELECT destinations.* FROM destinations INNER JOIN trips_destinations ON destinations.id = trips_destinations.destination_id WHERE trips_destinations.trip_id = $1',
            [trip_id]
        )
